...
def show_overview(agent_status):
    ...
    if agent_status:
        ...
        with col2:
            goals = agent_status.get('active_goals', 0)
            st.metric("🎯 Активные Цели", goals)
        
        with col3:
            memories = agent_status.get('episodes_in_memory', 0)
    ...
    if agent_status and 'modules' in agent_status:
...
def main():
    ...
    tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs([
        "📊 Обзор", "💬 Чат", "🔧 Ollama", "💾 Память", "🧠 Сознание", "🎯 Цели", "🌍 Мир"
    ])
    ...
    with tab6:
        ...
        if agent_status and 'goals' in agent_status:
            goals = agent_status['goals']
            for goal in goals:
                with st.expander(f"🎯 {goal.get('description', 'Цель')}"):
                    st.write(f"**Приоритет:** {goal.get('priority', 'Неизвестно')}")
                    st.write(f"**Прогресс:** {goal.get('progress', 0):.1f}%")
                    st.write(f"**Статус:** {'Активна' if goal.get('active', False) else 'Неактивна'}")
        else:
            st.info("Цели не загружены")
    
    with tab7:
        st.header("🌍 Модель Мира")
        if agent_status and 'world_model' in agent_status:
            world_model = agent_status['world_model']
            st.json(world_model)
        else:
            st.info("Модель мира не загружена")
